...
actor WhisperSTT {
  ...
  func transcribe(
    ...
    while seek < contentFrames {
      ...
      // This catches hallucinations where model generates impossible timestamps (e.g., 60s in a 5s segment)
      // Use generous tolerance for short clips where Whisper's timing is less accurate
      currentSegments = currentSegments.filter { segment in
        let relativeEnd = Float(segment.end) - timeOffset
        // For short clips, Whisper timestamps can overshoot significantly
        // Use minimum 5s tolerance, or 2x segment duration for longer clips
        let tolerance = max(5.0, segmentDuration)
        if relativeEnd > segmentDuration + tolerance {
          Log.model.warning("Filtering hallucinated segment (timestamp \(String(format: "%.1f", relativeEnd))s exceeds \(String(format: "%.1f", segmentDuration + tolerance))s threshold): '\(segment.text.prefix(50))'")
          return false
        }
        return true
      }
