...
final class KokoroTokenizer {
  ...
  private var isLexiconLoaded = false
  ...
  /// Set pre-loaded lexicons.
  /// Call this before phonemization for English voices.
  func setLexicons(us: [String: String], gb: [String: String]) {
    cachedUSLexicon = us.isEmpty ? nil : us
    cachedGBLexicon = gb.isEmpty ? nil : gb
    isLexiconLoaded = true
  }

  /// Check if lexicons are loaded.
  var lexiconsLoaded: Bool {
    isLexiconLoaded
  }
  ...
  private func removeCommasFromNumbers(_ text: String) -> String {
    Self.commaInNumberRegex.stringByReplacingMatches(
      in: text,
  ...
  private func tokenize(tokens: [String], features: [String: Any], nonStringFeatures _: Set<String>) throws -> [Token] {
    var result: [Token] = []
  ...
  private func generatePhoneme(for token: String) throws -> String {
    ...
    if isLexiconEnabled {
      let lexicon: [String: String]? = switch currentLanguage {
        case .enUS:
          cachedUSLexicon
        case .enGB:
          cachedGBLexicon
  ...
  private func isContentWord(_ word: String) -> Bool {
    !isFunctionWord(word) && word.count > 2 && isAlphabetic(word)
  }
