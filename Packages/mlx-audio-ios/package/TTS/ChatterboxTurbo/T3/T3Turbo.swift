...
/// Cached KV state for conditioning prefill
/// Allows reusing the conditioning computation across multiple generation calls
struct T3TurboConditioningCache: @unchecked Sendable {
  /// KV states for each layer after conditioning prefill (array of [keys, values] per layer)
  let layerStates: [[MLXArray]]
  /// Length of the conditioning sequence
  let conditioningLength: Int

  init(layerStates: [[MLXArray]], conditioningLength: Int) {
    self.layerStates = layerStates
    self.conditioningLength = conditioningLength
  }
}
...
struct T3TurboCond: @unchecked Sendable {
  ...
  /// Cached KV states from conditioning prefill (optional, for performance)
  var conditioningCache: T3TurboConditioningCache?

  init(
    speakerEmb: MLXArray,
    condPromptSpeechTokens: MLXArray? = nil,
    condPromptSpeechEmb: MLXArray? = nil,
    conditioningCache: T3TurboConditioningCache? = nil
  ) {
...
class T3Turbo: Module, @unchecked Sendable {
  ...
  /// Prefill conditioning through the transformer and cache the KV states.
  /// This allows reusing the conditioning computation across multiple generation calls.
  ///
  /// - Parameter t3Cond: Conditioning information (will be mutated to include cache)
  func prefillConditioning(_ t3Cond: inout T3TurboCond) {
    // Skip if already cached
    if t3Cond.conditioningCache != nil {
      Log.model.debug("[T3-Cache] Conditioning already cached, skipping prefill")
      return
    }

    // Prepare conditioning embeddings
    let condEmb = prepareConditioning(&t3Cond)
    let condLen = condEmb.shape[1]

    Log.model.info("[T3-Cache] Prefilling conditioning: \(condLen) tokens through \(self.tfmr.h.count) layers")

    // Create fresh cache
    let cache = tfmr.newCache()

    // Run conditioning through transformer (prefill)
    _ = tfmr(inputsEmbeds: condEmb, cache: cache)
    ...
    // Extract and save KV states from each layer
    var layerStates: [[MLXArray]] = []
    for c in cache {
      if let simple = c as? KVCacheSimple {
        layerStates.append(simple.state)
      }
    }

    t3Cond.conditioningCache = T3TurboConditioningCache(
      layerStates: layerStates,
      conditioningLength: condLen
    )

    Log.model.info("[T3-Cache] Cached \(layerStates.count) layer KV states")
  }

  /// Restore cached conditioning KV states into a fresh cache
  private func restoreCachedConditioning(cache: [any KVCache], from condCache: T3TurboConditioningCache) {
    for (i, c) in cache.enumerated() {
      if let simple = c as? KVCacheSimple, i < condCache.layerStates.count {
        let savedState = condCache.layerStates[i]
        if savedState.count == 2 {
          simple.update(keys: savedState[0], values: savedState[1])
        }
      }
    }
  }
  ...
  func inferenceTurbo(
    ...
    // Initial forward pass - use cached conditioning if available
    var hiddenStates: MLXArray
    if let condCache = t3Cond.conditioningCache {
      // Restore cached conditioning KV states
      restoreCachedConditioning(cache: cache, from: condCache)

      // Only process text + speech embeddings (conditioning already in cache)
      let textEmbeddings = textEmb(tokens)
      let speechEmbeddings = speechEmb(speechStartToken)
      let inputEmbeds = MLX.concatenated([textEmbeddings, speechEmbeddings], axis: 1)

      Log.model.debug("[T3-Cache] Using cached conditioning (\(condCache.conditioningLength) tokens), processing \(inputEmbeds.shape[1]) new tokens")
      hiddenStates = tfmr(inputsEmbeds: inputEmbeds, cache: cache)
    } else {
      // No cache - prepare full embeddings including conditioning
      let (embeds, _) = prepareInputEmbeds(
        t3Cond: &t3Cond,
        textTokens: tokens,
        speechTokens: speechStartToken
      )
      hiddenStates = tfmr(inputsEmbeds: embeds, cache: cache)
    }
