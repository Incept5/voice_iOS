...
actor ChatterboxTurboTTS {
  ...
  ///
  /// Chatterbox works best with longer chunks for more natural prosody.
  /// With conditioning KV caching, we have more headroom since conditioning tokens
  /// (~376) are no longer re-processed per chunk. 500 characters (~125 text tokens)
  /// typically generates ~300-400 speech tokens, well within the 1000 token limit.
  private static let maxChunkCharacters = 500
  ...
  func generate(
    ...
    for chunk in chunks {
      ...
      // Note: Caller should manage memory clearing if needed
    }
